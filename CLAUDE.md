...
- **`useTodo(date)`** — React Query read + Firebase write for `todos/{date}`. **No `onMutate` optimistic update** — `SidebarTodo` owns local state for immediate UI feedback, so the hook only exposes `{ items, isLoading, save }`. `onSettled` invalidates **both** `['todo', date]` and `['todoHistory']`: this is safe because `SidebarTodo` never re-initializes `localItems` from `query.data` after the first load (`initializedRef` guard), so the refetch only refreshes the cache for future remounts. Previously had `onMutate` with `cancelQueries` + `setQueryData`; removed because async `onMutate` caused race conditions (second add / checkbox uncheck failed when called before `setQueryData` resolved).
